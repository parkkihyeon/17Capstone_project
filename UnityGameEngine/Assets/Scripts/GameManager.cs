...
public class GameManager : MonoBehaviour
{
    ...
    char[][] board = {  new char[] { '1', '2', '3', '5', 'X', '5', '3', '2', '1' },
    ...
    public GameObject Status; // "상대를 기다리고 있습니다" 텍스트 오브젝트
            ...
            if (click) // 병사를 클릭했든 Board를 클릭했든 둘중 하나
            {
                ...
                else // 목표지점 선택시
                {
                    ...
                    else // 클릭한 병사가 아닌 이동할 위치의 Board를 클릭한 경우
                    {
                        ...
                        //ConnectServer.Moving = true; // 병사이 이동중임을 나타낸다.
                        ConnectServer.Instance.SendUnitBoard(1,2,3,4);
                    }
    ...
    public void MovePlayer(int from, int to) // 실제로 병사를 이동시켜주는 함수
    {
        int x = from % 9, y = from / 9; // offset값을 좌표로 변환한다.

        Cell[y][x].Player.MoveTo(Cell[to / 9][to % 9]); // from 위치에 있는 병사를 이동시킨다.
        Cell[y][x].Player = null; // 현재 위치에 있는 병사가 다른 위치로 이동할 것이므로 현재 위치의 오브젝트를 초기화해준다.
        Cell[y][x].Type = SoldierType.NONE; // 현재 위치의 병사 타입도 NONE으로 초기화
